...
# Load dataset
df = load_data('Dataset/supply_chain_data.csv')

# ---------------------------------------------------------
# NAVIGATION
# ---------------------------------------------------------
st.sidebar.title("🧭 Navigation")
page = st.sidebar.radio("Go to", ["📈 Dashboard", "🤖 Prediction Results"])

# ---------------------------
# DASHBOARD (original code) — executed only when Dashboard page selected
# I kept your original code intact, only indented inside this if-block.
# ---------------------------
if page == "📈 Dashboard":
    # Sidebar for filters
    ...
    st.markdown(
        ...
        <div style='text-align: center; padding: 10px;'>
            <p style='font-size: 1em; font-family: "Inter", sans-serif; color: #ffffff;'>
                Github Link -  <a href='https://github.com/Ashuytosh/Supply_Chain_Analysis_and_Prediction' target='_blank' style='color: #22d3ee;'>Click Here</a>
            </p>
        </div>
        """,
        unsafe_allow_html=True
...
# ---------------------------------------------------------
# PREDICTION RESULTS PAGE (updated modern layout)
# ---------------------------------------------------------
elif page == "🤖 Prediction Results":
    # HEADER
    st.markdown(
        """
        <div style='text-align: center; background: linear-gradient(135deg, #0f766e, #14b8a6); 
                    padding: 25px; border-radius: 12px;'>
            <h1 style='font-size: 2.5em; font-weight: 700; color: #ffffff;'>
                📈 Revenue Prediction Results
            </h1>
            <p style='color: #e0f2f1; font-size: 1.2em;'>Model Performance Summary & Insights</p>
        </div>
        """, unsafe_allow_html=True
    )
    ...
    results_data = {
        ...
        "Train R²": [0.991831, 0.999961, 0.901093],
        "Test R²": [0.985340, 0.979490, 0.727305],
        "Train MAE": [197.307167, 12.999400, 676.808520],
        "Test MAE": [295.941824, 351.525849, 1280.351236],
        "Train RMSE": [242.905198, 16.715504, 845.199404],
        "Test RMSE": [343.237855, 405.983410, 1480.340807],
        "Test MAPE (%)": [6.982042, 8.595390, 32.571938]
    }
    ...
    styled_table = results_df.style.set_properties(**{
        ...
        {'selector': 'thead th', 'props': [('background-color', '#14b8a6'), ('color', 'white'), ('font-size', '1.1em')]}
    ])

    st.dataframe(styled_table, use_container_width=True)

    st.divider()
    st.markdown("## 🔍 Linear Regression: Actual vs Predicted Revenue")

    # --- Actual vs Predicted Plot ---
    actual = np.linspace(1000, 10000, 50)
    predicted = actual + np.random.normal(0, 800, 50)
    ...
    fig.update_layout(
        title="Actual vs Predicted Revenue (Linear Regression)",
        template='plotly_dark',
    ...
    features = ['Lead times', 'Manufacturing costs', 'Order quantities', 'Defect rates', 'Shipping costs', 'Availability', 'Stock levels']
    rf_importance = np.random.rand(len(features))
    xgb_importance = np.random.rand(len(features))

    col1, col2 = st.columns(2)
    with col1:
        fig = px.bar(x=features, y=rf_importance, title="Feature Importance (Random Forest)", color=rf_importance, color_continuous_scale='Tealgrn')
        fig.update_layout(template='plotly_dark')
        st.plotly_chart(fig, use_container_width=True)
    with col2:
        fig = px.bar(x=features, y=xgb_importance, title="Feature Importance (XGBoost)", color=xgb_importance, color_continuous_scale='Viridis')
        fig.update_layout(template='plotly_dark')
        st.plotly_chart(fig, use_container_width=True)
    ...
    st.markdown(
        ...
        <div style='background: #1e293b; padding: 25px; border-radius: 12px;'>
            <h3 style='color: #22d3ee;'>📈 Business Recommendations</h3>
            <ul style='color: #f8fafc; font-size: 1.1em; line-height: 1.6;'>
                <li>Focus on reducing lead times and manufacturing costs — top revenue drivers.</li>
                <li>Optimize supplier performance to control defect rates.</li>
                <li>Refine transport mode allocation to reduce logistics costs.</li>
                <li>Regularly retrain predictive models with new data.</li>
                <li>Integrate predictive metrics into operational dashboards.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True
    )
